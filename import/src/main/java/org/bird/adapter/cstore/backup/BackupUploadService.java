...
public class BackupUploadService implements IBackupUploadService {
    ...
    public class HealthcareDestinationUploadAsyncJob extends UploadAsyncJob {
        ...
        public void run() {
            try {
                InputStream inputStream = readBackupExceptionally();
                try {gatwayClient.stowRs(inputStream);} catch (IOException e){}
                logSuccessUpload();
            } catch (IGatewayClient.DicomGatewayException dwe) {
                logUploadFailed(dwe);
    ...
    private void throwOnHttpFilterFail(IGatewayClient.DicomGatewayException dwe, int httpCode) throws CompletionException {
    ...
    private void throwOnNoResendAttemptsLeft(IGatewayClient.DicomGatewayException dwe, String uniqueFileName) throws CompletionException {
        throw new CompletionException(getNoResendAttemptLeftException(dwe, uniqueFileName));
    }


    private BackupException getNoResendAttemptLeftException(IGatewayClient.DicomGatewayException dwe, String uniqueFileName) {
